...
check_latest_version(){
	...
	# 优先走镜像访问 GitHub API，失败再回退官方直连
	local api_url_mirror="${GH_MIRROR}${api_url}"
	local api_result
	api_result="$($downloader - "$api_url_mirror" 2>/dev/null)"
	if [ -z "$api_result" ]; then
		echo "镜像 API 访问失败，回退到 GitHub 官方 API..."
...
update_core(){
	...
	echo "正在获取下载链接..."
	mkdir -p /tmp/run
	# 按通道生成下载源：镜像 GitHub → 直连 GitHub → static.adguard.com（垫底）
	local github_url="https://github.com/AdguardTeam/AdGuardHome/releases/download/${latest_ver}/AdGuardHome_linux_${Arch}.tar.gz"
	if [ "$release_channel" = "beta" ]; then
		local static_url="https://static.adguard.com/adguardhome/beta/AdGuardHome_linux_${Arch}.tar.gz"
	else
		local static_url="https://static.adguard.com/adguardhome/release/AdGuardHome_linux_${Arch}.tar.gz"
	fi
	{
		echo "${GH_MIRROR}${github_url}"
		echo "${github_url}"
		echo "${static_url}"
